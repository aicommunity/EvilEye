...
def test_format_camera_event_information_uses_names():
    assert format_camera_event_information("Cam1", connected=True) == "Camera=Cam1 reconnect"
    assert format_camera_event_information("Cam1", connected=False) == "Camera=Cam1 disconnect"
...
def test_sanitize_camera_event_record_drops_credentials_and_resolves_name():
    ...
    assert clean["information"] == "Camera=Cam5 reconnect"
