...
import sys
import os
def test_journal_columns_compatibility(journal_test_logger, qapp):
    """Test that JSON journal columns match database journal structure"""
    
    journal_test_logger.info("=== Test Journal Columns Compatibility ===")
    ...
    try:
        ...
        if journal.table.columnCount() == 7:
            journal_test_logger.info(f"✅ Table has {journal.table.columnCount()} columns (matches database journal)")
            ...
            if headers == expected_headers:
                journal_test_logger.info("✅ Column headers match database journal structure")
                for i, header in enumerate(headers):
                    journal_test_logger.info(f"   Column {i}: {header}")
            else:
                journal_test_logger.info("❌ Column headers don't match expected structure")
                journal_test_logger.info(f"   Expected: {expected_headers}")
                journal_test_logger.info(f"   Actual: {headers}")
        else:
        ...
        if journal.table.rowCount() > 0:
            ...
            if name_item and name_item.text() == 'Cam1':
                journal_test_logger.info("✅ Name column contains source_name: Cam1")
            else:
                journal_test_logger.error(f"❌ Name column contains: {name_item.text() if name_item else 'None'}")
            
            if event_item and event_item.text() == 'Event':
                journal_test_logger.info("✅ Event column contains 'Event' (matches database journal)")
            else:
                journal_test_logger.error(f"❌ Event column contains: {event_item.text() if event_item else 'None'}")
            
            if info_item and 'Object Id=1' in info_item.text():
                journal_test_logger.info("✅ Information column contains object details")
            else:
                journal_test_logger.error(f"❌ Information column contains: {info_item.text() if info_item else 'None'}")
            ...
            if lost_preview_item:
                journal_test_logger.info("✅ Lost preview column exists")
            else:
                journal_test_logger.info("❌ Lost preview column missing")
        else:
        ...
        journal_test_logger.info("\n📋 Summary:")
        journal_test_logger.info("   ✅ JSON journal has same column structure as database journal")
        journal_test_logger.info("   ✅ All 7 columns present: Name, Event, Information, Time, Time lost, Preview, Lost preview")
        journal_test_logger.info("   ✅ Column headers match database journal format")
        journal_test_logger.info("   ✅ Data is loaded correctly from JSON files")
        journal_test_logger.info("   ✅ Source name is displayed in Name column")
        journal_test_logger.info("   ✅ Event column shows 'Event' (matches database journal)")
