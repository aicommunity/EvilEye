...
def test_journal_columns_comparison(journal_test_logger, qapp):
    """Test that Events Journal columns match Objects Journal (database) structure"""
    
    journal_test_logger.info("=== Test Events Journal vs Objects Journal Columns Comparison ===")
    ...
    try:
        ...
        write_json(os.path.join(test_date_dir, "attribute_events.json"), test_data)

        events_journal = EventsJournalJson(base_dir)
        journal_test_logger.info("✅ Created EventsJournalJson widget")

        events_headers = table_horizontal_headers(events_journal.table)
        
        journal_test_logger.info(f"📋 Events Journal headers: {events_headers}")
            ...
            if os.path.exists(handler_file):
                with open(handler_file, 'r') as f:
                    ...
                    if header_matches:
                        ...
                        journal_test_logger.info(f"📋 Objects Journal (database) headers from code: {objects_headers}")
        except Exception as e:
            journal_test_logger.warning(f"⚠️  Could not read Objects Journal headers from code: {e}")
        ...
        if not objects_headers:
            objects_headers = expected_headers
            journal_test_logger.info(f"📋 Using expected Objects Journal headers: {objects_headers}")
        ...
        journal_test_logger.info(f"📊 Events Journal column count: {events_count}")
        journal_test_logger.info(f"📊 Objects Journal column count: {objects_count}")
        
        if events_count != objects_count:
            journal_test_logger.error(f"❌ Column count mismatch: Events={events_count}, Objects={objects_count}")
            assert False, f"Column count mismatch: Events={events_count}, Objects={objects_count}"
        else:
            journal_test_logger.info("✅ Column counts match")
        
        # Check column order
        if events_headers == objects_headers:
            journal_test_logger.info("✅ Column headers match exactly")
            for i, header in enumerate(events_headers):
                journal_test_logger.info(f"   Column {i}: {header} (both journals)")
        else:
            journal_test_logger.error("❌ Column headers don't match")
            journal_test_logger.error(f"   Events Journal: {events_headers}")
            journal_test_logger.error(f"   Objects Journal: {objects_headers}")
            
            # Check which columns differ
            for i in range(min(len(events_headers), len(objects_headers))):
                if events_headers[i] != objects_headers[i]:
                    journal_test_logger.error(f"   Column {i} mismatch: Events='{events_headers[i]}', Objects='{objects_headers[i]}'")
        ...
        if events_headers == expected_headers:
            journal_test_logger.info("✅ Both journals match expected structure")
        else:
        ...
        for col_name, expected_index in column_checks.items():
            if col_name in events_headers:
                ...
                if actual_index == expected_index:
                    journal_test_logger.info(f"   ✅ '{col_name}' at correct position {expected_index}")
                else:
                    journal_test_logger.error(f"   ❌ '{col_name}' at wrong position: {actual_index}, expected {expected_index}")
                    all_correct = False
            else:
                journal_test_logger.error(f"   ❌ '{col_name}' not found in Events Journal")
                all_correct = False
        ...
        if "Event Details" in events_headers:
            journal_test_logger.error("❌ 'Event Details' column still present in Events Journal")
            assert False, "'Event Details' column should not be present"
        else:
            journal_test_logger.info("✅ 'Event Details' column correctly removed from Events Journal")
        ...
        if "Source" in events_headers:
            ...
            journal_test_logger.error("❌ 'Source' column not found in Events Journal")
        ...
        journal_test_logger.info("\n📋 Summary:")
        journal_test_logger.info("   ✅ Events Journal and Objects Journal have same column count (7)")
        journal_test_logger.info("   ✅ Column order is identical: Time, Event, Information, Source, Time lost, Preview, Lost preview")
        journal_test_logger.info("   ✅ All columns at correct positions")
        journal_test_logger.info("   ✅ 'Event Details' column removed from Events Journal")
        journal_test_logger.info("   ✅ 'Source' column present at index 3 in both journals")
