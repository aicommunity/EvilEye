...
@pytest.mark.usefixtures("require_gst")
def test_gstreamer_video_loop_reconnect_single(
    test_video_mp4,
    tmp_path: Path,
):
    """
    Тест реконнекта одного видеофайла с loop_play при завершении видео.
    Проверяет, что после EOS видео перезапускается и продолжает отправлять кадры.
    """
    ...
    # Отключить GLib main loop для теста
    try:
    ...
    assert len(frames_before_eos) > 0, "Должны быть получены кадры до EOS"
    initial_frame_count = len(frames_before_eos)
    
    # Ждем завершения видео (EOS) и перезапуска
    # Видео длится ~3 секунды, ждем немного больше для обработки EOS
    time.sleep(5.0)
    
    # Проверяем, что после EOS видео перезапустилось и продолжает отправлять кадры
    frames_after_eos = []
    ...
    # Проверки
    assert reconnect_detected, "Реконнект должен был произойти после EOS"
    ...
    # Проверяем, что получили кадры из нового цикла (frame_id должен увеличиваться)
    if frames_after_eos:
        last_frame_before = frames_before_eos[-1] if frames_before_eos else None
        first_frame_after = frames_after_eos[0]
        
        if last_frame_before and hasattr(first_frame_after, 'frame_id'):
            # После перезапуска frame_id может начаться заново или продолжиться
            # Главное - что кадры продолжают приходить
            assert hasattr(first_frame_after, 'image'), "Кадр должен содержать изображение"


@pytest.mark.usefixtures("require_gst")
def test_gstreamer_video_loop_reconnect_multiple(
    test_video_mp4,
    tmp_path: Path,
):
    """
    Тест реконнекта множественных источников с loop_play.
    ...
    # Ждем завершения видео (EOS) для всех источников
    time.sleep(5.0)
