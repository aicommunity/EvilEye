...
2. Use HTTPS (`evileye install-server` TLS wizard or `--ssl-certfile` / `--ssl-keyfile`). `web_auth.secure_cookies` is auto-enabled when SSL cert env/files are set. Do **not** enable HSTS (`server.hsts` / `EVILEYE_HSTS=1`) for LAN self-signed certs — only after clients trust the CA or you use a public certificate.
