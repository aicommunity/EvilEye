...
### Рекомендуемый operational-сценарий

Основной сценарий эксплуатации:

- запускать саму систему через `evileye run`
- при необходимости включать server module из конфигурации системы
- использовать web/API как представление текущего состояния уже запущенной системы

В этом сценарии именно система является владельцем жизненного цикла runtime, а серверный модуль работает как ее веб-интерфейс и API-слой.

Сценарий `evileye server` сохраняется как служебный режим для отладки, удаленного управления и контролируемого запуска через REST API, но не является приоритетным operational-path.
...
Этот режим является служебным. Для основного пользовательского сценария рекомендуется запускать систему через `evileye run`, а веб-сервер поднимать как модуль самой системы.
