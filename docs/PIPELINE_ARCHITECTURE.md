...
7. Attributes ROI (`RoiFeeder` - извлечение ROI из bbox первичных объектов)
8. Attribute Classifier (`AttributeClassifier` - классификация атрибутов на ROI)
...
### Управление классами и маппинг

Система использует централизованную систему управления классами через `ClassManager` для сопоставления имён классов и их ID. Это позволяет:

- **Единообразное именование**: все компоненты (детекторы, трекеры, обработчики объектов) используют одинаковые имена классов
- **Гибкость конфигурации**: можно указывать классы как по имени (`primary_by_name: ["person"]`), так и по ID (`primary_by_id: [0]`)
- **Автоматическое преобразование**: `ClassManager` автоматически преобразует имена в ID при инициализации компонентов

Если `ClassManager` не настроен, система использует fallback на словарь `class_mapping` (обычно из конфигурации детектора), который сопоставляет имена классов с их ID. Это обеспечивает обратную совместимость со старыми конфигурациями.

**Пример использования:**
- В конфигурации указываются имена: `"primary_by_name": ["person", "car"]`
- `ClassManager` преобразует их в ID: `[0, 2]` (если это соответствует модели детектора)
- Компоненты (`RoiFeeder`, `ObjectsHandler`) получают уже преобразованные ID и используют их для фильтрации объектов
