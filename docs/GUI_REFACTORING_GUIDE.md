...
#### 5. Database Journal (`db_journal.py`, `journal_init_thread.py`)
Журнал событий и объектов с поддержкой базы данных PostgreSQL.

**Архитектура инициализации:**
- **`JournalInitThread`** - отдельный поток для асинхронной инициализации `DatabaseControllerPg` и подключения к БД
- **`DatabaseJournalWindow`** - главное окно журнала, которое создаётся после успешной инициализации БД
- Безопасная работа с `db_controller`: все атрибуты инициализируются явно в конструкторе (`db_controller = None`, `database_available = False`), что устраняет необходимость в проверках `hasattr`
...
- Асинхронная инициализация через отдельный поток для предотвращения блокировки GUI

**Функциональность:**
- Отображение событий found и lost
- Превью изображений для событий
- Асинхронная инициализация БД без блокировки интерфейса
- Прогресс-бар и статусные сообщения во время инициализации
