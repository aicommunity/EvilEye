from core import EvilEyeBase, ProcessorSource, ProcessorFrame, ProcessorStep, ProcessorBase
...
class PipelineSurveillance(Pipeline):
    ...
    def init_impl(self, **kwargs):
        ...
        self._init_encoders(self.get_processor_params("trackers"))
        
        # Initialize processors in surveillance-specific order
        self._init_sources(self.get_processor_params("sources"), self._credentials)
        self._init_preprocessors(self.get_processor_params("preprocessors"))
        self._init_detectors(self.get_processor_params("detectors"))
        self._init_trackers(self.get_processor_params("trackers"))
        self._init_mc_trackers(self.get_processor_params("mc_trackers"))
    ...
    def _init_sources(self, params: List[Dict], credentials: Dict):
        ...
        sources_proc = ProcessorSource(processor_name="sources", class_name="VideoCapture", num_processors=num_sources, order=0)
    ...
    def _init_trackers(self, params: List[Dict]):
        ...
        trackers_proc = ProcessorStep(processor_name="trackers", class_name="ObjectTrackingBotsort", num_processors=num_trackers, order=3)
    ...
    def _init_mc_trackers(self, params: List[Dict]):
        ...
        num_trackers = len(params)
        mc_trackers_proc = ProcessorStep(processor_name="mc_trackers", class_name="ObjectMultiCameraTracking", num_processors=num_trackers, order=4)
