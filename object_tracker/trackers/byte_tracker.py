...
class BYTETracker:
    ...
    def update(
        ...
        Args:
            cls (np.ndarray): Array of class indecies of new detections with shape (n,)
            bboxes_xywh (np.ndarray): Array of box coordinates  of new detections with shape (n, 4).
                                      Format - [xc, yc, w, h] (absolute values)
            scores (np.ndarray): Array of scores (confidences) of new detections with shape (n,)
            img (np.ndarray, optional): Image that contains the new detections. Defaults to None.

        Returns:
            np.ndarray: Array of current tracks with shape (n, ...)
        """
        self.frame_id += 1
        activated_stracks = []
        refind_stracks = []
