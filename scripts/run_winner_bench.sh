#!/usr/bin/env bash
# Final 5×180 bench using WINNER.txt env from backlog matrix.
set -euo pipefail
...
OUT_DIR="${OUT_DIR:-reports/poly_videos_mode_compare}"
PYTHON="${PYTHON:-python3}"
WINNER_FILE="$MATRIX_ROOT/WINNER.txt"

if [[ ! -f "$WINNER_FILE" ]]; then
  echo "WINNER.txt not found. Run: ./scripts/run_backlog_matrix.sh all" >&2
  exit 1
fi
...
"$PYTHON" scripts/collect_poly_mode_output_snapshot.py
...
"$PYTHON" scripts/compare_poly_bench_runs.py \
  ...
  --current-dir "$OUT_DIR" \
  --out docs/mp_fps_phase2_summary.md

echo "Done. Winner=$EXP"
