...
class ObjectDetectorRtdetr(ObjectDetectorBase):
    id_cnt = 0  # Переменная для присвоения каждому детектору своего идентификатора

    def __init__(self):
        super().__init__()
        self.model_name = "rtdetr-l.pt"  # По умолчанию используем RT-DETR-L
    ...
    def init_impl(self):
        ...
        for i in range(self.num_detection_threads):
            ...
            thread = DetectionThreadRtdetr(model_path, self.stride, self.classes, self.source_ids, self.roi, inf_params,
                                         self.queue_out)
            thread.start()
