...
class StreamPlayerWindow(QMainWindow):
    ...
    def _on_date_selected(self, date: str):
        ...
        if self._selected_cameras:
            ...
            self._load_camera_segments()
            self.video_grid.set_cameras(self._selected_cameras, self._camera_segment_times, self._source_config,
                                        self.base_dir, date)
            self._configure_metadata_for_players()
        else:
            self._configure_metadata_for_players()
    ...
    def _load_camera_segments(self):
        ...
        for camera in self._selected_cameras:
            ...
            if folder_name != camera:
                self.logger.info(
                    f"Loaded {len(valid_segments)} valid segments (out of {len(all_segments)} total) for source '{camera}' from folder '{folder_name}'")
            else:
                self.logger.info(
                    f"Loaded {len(valid_segments)} valid segments (out of {len(all_segments)} total) for camera {camera}")
            ...
            if not has_recording:
                ...
                if next_time:
                    ...
                    if abs(new_position_ms - self._current_position_ms) > 1000:  # Разница больше 1 секунды
                        ...
                        self.logger.debug(
                            f"Jumped to next available time: {next_time}, position_ms={self._current_position_ms}")
    ...
    def save_state(self):
        ...
        state = {
            ...
            'grid_rows': getattr(self.camera_selector, 'rows_spin',
                                 None) and self.camera_selector.rows_spin.value() or 2,
            'grid_cols': getattr(self.camera_selector, 'cols_spin',
                                 None) and self.camera_selector.cols_spin.value() or 2,
            'last_date': self.camera_selector.get_selected_date() if hasattr(self.camera_selector,
                                                                             'get_selected_date') else None,
            'grid_cell_sources': getattr(self.video_grid, '_grid_cell_sources', {}).copy(),
            'show_metadata': getattr(self, 'show_metadata_checkbox',
                                     None) and self.show_metadata_checkbox.isChecked() or False
        }
