...
class CameraSelectorWidget(QWidget):
    ...
    def _load_cameras_for_date(self, date: str):
        ...
        self.logger.info(
            f"Loaded {len(camera_folders)} camera folders and {len(self._available_sources)} sources for date {date}")
...
class VideoGridWidget(QWidget):
    ...
    def set_cameras(self, cameras: List[str], camera_segment_times: Dict[str, List[Tuple]], source_config: Dict = None,
                    base_dir: str = None, date_folder: str = None):
        ...
        for camera_folder in unique_camera_folders:
            ...
            if not segments_to_use or len(segments_to_use) == 0:
                ...
                if row >= 0 and col >= 0:
                    ...
                    label.setStyleSheet(
                        "background-color: rgba(0, 0, 0, 200); color: white; padding: 3px; font-weight: bold;")
                    ...
                    message_widget.setStyleSheet(
                        "background-color: black; color: white; font-size: 16px; padding: 20px;")
                    ...
                    self.logger.info(
                        f"Created placeholder widget for {camera_folder} at row={row}, col={col}, grid_idx={grid_idx}")
            ...
            if is_split:
                ...
                if segments_to_use:
                    first_segment = segments_to_use[0][2] if isinstance(segments_to_use[0], tuple) else segments_to_use[
                        0]
                        ...
                        if config_result:
                            ...
                            for i in range(num_split):
                                ...
                                if region_row < 0 or region_col < 0:
                                    self.logger.warning(
                                        f"No free position in grid for split region {i} of {camera_folder}")
                                    continue
                                ...
                                if i < len(split_player._region_widgets):
                                    ...
                                    source_name = region_info.get('source_name', source_names[i] if i < len(
                                        source_names) else f"Source{i}")
                                    ...
                                    region_container.setSizePolicy(QSizePolicy.Policy.Preferred,
                    ...
                    self.logger.info(
                        f"No video segments found for split camera folder: {camera_folder}, creating placeholder widgets")
                    num_split = split_config.get('num_split', 1)
                    source_names = split_config.get('source_names', [])
                    ...
                    for i in range(num_split):
                        ...
                        self.logger.info(
                            f"Created placeholder widget for split region {i} ({source_name}) of {camera_folder} at row={region_row}, col={region_col}, grid_idx={grid_idx}")
            else:
                ...
                container_widget.setMinimumSize(100,
                                                100)  # Минимальный размер для предотвращения слишком маленьких виджетов
                ...
                if camera_segment_times.get(camera_name):
                    ...
                    if os.path.exists(first_segment) and os.path.getsize(first_segment) > 1024:
                        ...
                        self.logger.warning(
                            f"Video file not found or too small for camera {camera_name}: {first_segment}")
    ...
    def play_all(self):
        ...
        for camera_folder, player in self._video_players.items():
            player_type = "SplitVideoPlayerWidget" if isinstance(player,
                                                                 SplitVideoPlayerWidget) else "VideoPlayerWidget"
                    ...
                    if segments and len(segments) > 0:
                        ...
                        if os.path.exists(first_segment) and os.path.getsize(first_segment) > 1024:
                            if player.play_video(first_segment):
                                ...
                                self.logger.warning(
                                    f"Failed to play video for camera_folder {camera_folder}: {first_segment}")
                                continue
                        else:
                            self.logger.warning(
                                f"Video file not found or too small for camera_folder {camera_folder}: {first_segment}")
                    else:
    ...
    def _find_segments_for_folder(self, camera_folder: str) -> Optional[List[Tuple]]:
        ...
        if split_config and split_config.get('split', False):
            ...
            if parent_folder and parent_folder in self._camera_segments:
                self.logger.debug(
                    f"Found segments for {camera_folder} via parent_folder {parent_folder} from camera_folder config")
        ...
        if sources_for_folder:
            ...
            if first_source_config and first_source_config.get('split', False):
                ...
                if parent_folder and parent_folder in self._camera_segments:
                    self.logger.debug(
                        f"Found segments for {camera_folder} via parent_folder {parent_folder} from first_source config")
                    return self._camera_segments[parent_folder]
    ...
    def seek_all(self, position_ms: int, should_play: bool = False):
        ...
        """
        self.logger.debug(
            f"seek_all called: position_ms={position_ms}, should_play={should_play}, _start_time={self._start_time}")
        ...
        self.logger.debug(
            f"seek_all: Calculated target_time={target_time} from _start_time={self._start_time} + {position_ms}ms")
        ...
        for camera_folder in self._video_players.keys():
            ...
            for idx, (start_time, end_time, path) in enumerate(segments):
                if start_time and end_time:
                    if start_time <= target_time < end_time:
                        ...
                        self.logger.debug(
                            f"seek_all for {camera_folder}: Found segment {idx} for target_time {target_time} (start={start_time}, end={end_time})")
                        break
                else:
            ...
            if target_segment_idx is None:
                if segments:
                    ...
                    self.logger.debug(
                        f"seek_all for {camera_folder}: No segment found for target_time {target_time}, first_start={first_start}, last_end={last_end}")
            ...
            if target_segment_idx is None:
                ...
                if segments and segments[0][0] and target_time < segments[0][0]:
                    ...
                    target_segment_idx = 0
                    self.logger.info(
                        f"target_time {target_time} is before first segment start {segments[0][0]} for {camera_folder}, using first segment at position 0")
                    ...
                    self.logger.info(
                        f"target_time {target_time} is after last segment end {segments[-1][1]} for {camera_folder}, showing 'No video available'")
                    ...
                    self.logger.debug(
                        f"No segment found for camera_folder {camera_folder} at time {target_time}, showing placeholder")
            ...
            self.logger.debug(
                f"seek_all for {camera_folder}: needs_reload={needs_reload} (in _no_video_cameras={camera_folder in self._no_video_cameras}), should_play={should_play}")
            if not needs_reload and player:
                ...
                if isinstance(player, SplitVideoPlayerWidget):
                    ...
                    if player._video_player:
                        ...
                        self.logger.debug(
                            f"seek_all for {camera_folder} (split): has_video_path={has_video_path}, is_playing={is_playing}, needs_reload={needs_reload}")
                    else:
                        needs_reload = True
                        self.logger.debug(
                            f"seek_all for {camera_folder} (split): _video_player missing, needs_reload=True")
                else:
                    ...
                    needs_reload = not has_video_path or not is_playing
                    self.logger.debug(
                        f"seek_all for {camera_folder} (regular): has_video_path={has_video_path}, is_playing={is_playing}, needs_reload={needs_reload}")
            ...
            if target_segment_idx != current_idx or needs_reload:
                ...
                if target_segment_idx == current_idx and needs_reload:
                    self.logger.debug(
                        f"seek_all for {camera_folder}: Segment index unchanged ({target_segment_idx}), but needs_reload=True, reloading segment: {new_segment}")
                        ...
                        if isinstance(player, SplitVideoPlayerWidget):
                            ...
                            if split_config:
                                ...
                                if player.set_split_config(split_config, new_segment):
                                    ...
                                    self._no_video_cameras.discard(
                                        camera_folder)  # Убрать из списка камер с черным экраном
                                    ...
                                    if player._video_player and player._video_player.timer and player._video_player.timer.isActive():
                                        ...
                                        self.logger.debug(
                                            f"seek_all for {camera_folder}: Stopped timer immediately after set_split_config()")
                                    ...
                                    if should_play:
                                        ...
                                        if was_no_video or not is_playing or not timer_active:
                                            self.logger.info(
                                                f"Explicitly starting playback for split player {camera_folder} after segment switch (was_no_video={was_no_video}, is_playing={is_playing}, timer_active={timer_active})")
                                            ...
                                            if was_no_video and timer_active and player._video_player and player._video_player.timer:
                                                self.logger.info(
                                                    f"Stopping timer for split player {camera_folder} before restart (was_no_video=True)")
                                                player._video_player.timer.stop()
                                            ...
                                            self.logger.info(
                                                f"Split player {camera_folder} already playing, no need to restart (was_no_video={was_no_video})")
                                else:
                                    self.logger.warning(
                                        f"Failed to switch split segment for camera_folder {camera_folder}: {new_segment}")
                            else:
                                self.logger.warning(
                                    f"No split config found for camera_folder {camera_folder}, cannot switch segment")
                        else:
                            # Обычный VideoPlayerWidget
                            ...
                            if player.play_video(new_segment):
                                ...
                                if player.timer and player.timer.isActive():
                                    player.timer.stop()
                                    self.logger.debug(
                                        f"seek_all for {camera_folder}: Stopped timer immediately after play_video()")
                                ...
                                if should_play:
                                    ...
                                    if was_no_video or not is_playing or (player_has_timer and not timer_active):
                                        self.logger.info(
                                            f"Explicitly starting playback for player {camera_folder} after segment switch (was_no_video={was_no_video}, is_playing={is_playing}, timer_active={timer_active})")
                                        if player_has_qmedia:
                                            ...
                                            # Даже если таймер активен, но камера показывала черный экран, перезапустить для надежности
                                                ...
                                                player.timer.start(interval)
                                                self.logger.info(
                                                    f"Restarted timer for player {camera_folder} after no-video state (interval={interval})")
                            else:
                                self.logger.warning(
                                    f"Failed to switch to segment for camera_folder {camera_folder}: {new_segment}")
                else:
                    self.logger.debug(
                        f"Segment file not found or invalid for camera_folder {camera_folder}: {new_segment}")
            ...
            if player:
                ...
                if segment_start:
                    ...
                    if segment_offset_ms < 0:
                        self.logger.info(
                            f"seek_all for {camera_folder}: target_time {target_time} is before segment_start {segment_start} (diff: {segment_offset_ms}ms), using position 0ms (camera started recording later)")
                        segment_offset_ms = 0
                        camera_has_started = False  # Камера еще не начала запись
                    else:
                        self.logger.debug(
                            f"seek_all for {camera_folder}: target_time {target_time}, segment_start {segment_start}, offset={segment_offset_ms}ms")
                else:
                    ...
                    if segment_offset_ms < 0:
                        segment_offset_ms = 0
                    self.logger.debug(
                        f"seek_all for {camera_folder}: No segment_start time, using position_ms={segment_offset_ms}ms")
                    ...
                    if isinstance(player, SplitVideoPlayerWidget):
                        if player._video_player and player._video_player.cap:
                            ...
                            if player._video_player.timer and player._video_player.timer.isActive():
                                player._video_player.timer.stop()
                                self.logger.debug(
                                    f"seek_all for {camera_folder}: Stopped timer before seek after reload")
                ...
                self.logger.info(
                    f"seek_all for {camera_folder}: Setting position to {segment_offset_ms}ms, should_play={should_play}, camera_has_started={camera_has_started}")
                    ...
                    if isinstance(player, SplitVideoPlayerWidget):
                        if player._video_player and player._video_player.cap and player._video_player.cap.isOpened():
                            ...
                            if abs(actual_frame - expected_frame) > 1:  # Допуск 1 кадр
                                self.logger.warning(
                                    f"seek_all for {camera_folder}: Position mismatch after seek - expected frame {expected_frame}, got {actual_frame}, retrying")
                ...
                if should_play:
                    ...
                    if isinstance(player, SplitVideoPlayerWidget):
                        ...
                        if player._video_player:
                            ...
                            self.logger.info(
                                f"seek_all for {camera_folder} (split): is_playing={is_playing}, timer_active={timer_active}")
                            if not is_playing or not timer_active:
                                self.logger.info(
                                    f"Resuming playback for split player {camera_folder} after seek (is_playing={is_playing}, timer_active={timer_active})")

                                # Убедиться, что _is_playing установлен перед вызовом play()
                                if not is_playing and player._video_player:
                                    player._video_player._is_playing = True
                                    self.logger.debug(
                                        f"seek_all for {camera_folder}: Set _video_player._is_playing=True before calling play()")
                                ...
                                if not timer_after_play and timer_before_play == False:
                                    self.logger.warning(
                                        f"seek_all for {camera_folder}: Timer did not start after play() (before={timer_before_play}, after={timer_after_play}, cap_opened={player._video_player.cap.isOpened() if player._video_player and player._video_player.cap else False})")
                                    ...
                                    if player._video_player and player._video_player.timer and player._video_player.cap and player._video_player.cap.isOpened():
                                        ...
                                        if hasattr(player._video_player,
                                                   '_timer_interval') and player._video_player._timer_interval:
                                            interval = player._video_player._timer_interval
                                        else:
                                            import cv2
                                            fps = player._video_player.cap.get(
                                                cv2.CAP_PROP_FPS) if player._video_player.cap else 30
                                            interval = int(1000 / fps) if fps > 0 else 33
                                        ...
                                        if timer_after_retry:
                                            self.logger.info(
                                                f"seek_all for {camera_folder}: Timer started successfully after direct start (interval={interval})")
                                        else:
                                            self.logger.error(
                                                f"seek_all for {camera_folder}: Timer failed to start even after direct start")
                                else:
                                    self.logger.debug(
                                        f"seek_all for {camera_folder}: Timer state after play() (before={timer_before_play}, after={timer_after_play})")
                        ...
                        self.logger.info(
                            f"seek_all for {camera_folder} (regular): is_playing={is_playing}, timer_active={timer_active}, has_qmedia={player_has_qmedia}")

                        if not is_playing or (player_has_timer and not timer_active):
                            # Если воспроизведение не запущено, запустить его
                            if player_has_qmedia:
                                self.logger.info(
                                    f"Resuming playback for player {camera_folder} (QMediaPlayer) after seek")
                                if pyqt_version == 6:
                                    player.player.play()
                                else:
                                    player.player.play()
                                self.logger.info(
                                    f"Resumed playback for player {camera_folder} (QMediaPlayer) after seek")
                            elif player_has_timer:
                                self.logger.info(
                                    f"Resuming playback for player {camera_folder} (OpenCV timer) after seek")
                                if not timer_active:
                                    # Убедиться, что _is_playing установлен
                                    if not player._is_playing:
                                        player._is_playing = True
                                        self.logger.debug(
                                            f"seek_all for {camera_folder}: Set _is_playing=True before starting timer")

                                    # Убедиться, что cap открыт
                                    if player.cap and not player.cap.isOpened():
                                        self.logger.warning(
                                            f"seek_all for {camera_folder}: cap not opened, cannot start timer")
                                    else:
                                        # Получить интервал таймера или использовать значение по умолчанию
                                        if hasattr(player, '_timer_interval') and player._timer_interval:
                                            interval = player._timer_interval
                                        ...
                                        self.logger.info(
                                            f"seek_all for {camera_folder}: Timer start attempt - before={timer_before_start}, after={timer_after_start}, interval={interval}, cap_opened={player.cap.isOpened() if player.cap else False}")

                                        # Проверить, что таймер действительно запустился
                                        if not timer_after_start:
                                            self.logger.warning(
                                                f"seek_all for {camera_folder}: Timer did not start, retrying with explicit interval={interval}")
                                            # Повторить попытку
                                            player.timer.stop()
                                            player.timer.start(interval)
                                            QApplication.processEvents()
                                            ...
                                            if not timer_after_retry:
                                                self.logger.error(
                                                    f"seek_all for {camera_folder}: Timer failed to start after retry (cap_opened={player.cap.isOpened() if player.cap else False}, _is_playing={player._is_playing})")
                                            else:
                                                self.logger.info(
                                                    f"seek_all for {camera_folder}: Timer started successfully after retry")
                                        else:
                                            self.logger.debug(
                                                f"seek_all for {camera_folder}: Timer started successfully (before={timer_before_start}, after={timer_after_start})")
                                else:
                                    self.logger.debug(
                                        f"seek_all for {camera_folder}: Timer already active, no need to start")
                                self.logger.info(
                                    f"Resumed playback for player {camera_folder} (OpenCV timer) after seek")
                        else:
                ...
                self.logger.warning(
                    f"seek_all for {camera_folder}: player is None, cannot set position or resume playback")
    ...
    def _seek_player(self, player, position_ms: int):
        ...
        if isinstance(player, SplitVideoPlayerWidget):
            ...
            self.logger.info(
                f"_seek_player for SplitVideoPlayerWidget: timer_active={timer_state_before}, cap_opened={cap_opened_before}")
            ...
            self.logger.info(
                f"_seek_player: Called seek({position_ms}) on SplitVideoPlayerWidget (timer: {timer_state_before} -> {timer_state_after}, cap: {cap_opened_before} -> {cap_opened_after})")
        elif player.player:
            # QMediaPlayer
            if pyqt_version == 6:
            ...
            if not player.cap.isOpened():
                self.logger.warning(
                    f"_seek_player: cap is not opened for player {type(player).__name__}, cannot set position")
                return
            ...
            if player.timer:
                ...
                if timer_state_before:
                    ...
                    player.timer.stop()
                    self.logger.debug(
                        f"_seek_player: Stopped timer before seek (was_active={timer_state_before}, _is_playing={player._is_playing})")
                else:
                    self.logger.debug(
                        f"_seek_player: Timer was not active before seek (_is_playing={player._is_playing})")
                ...
                if player.cap.isOpened():
                    ...
                    if isinstance(ret, tuple) and len(ret) == 2 and ret[0]:
                        ...
                        if actual_frame_after != frame_number:
                            ...
                            player.cap.set(cv2.CAP_PROP_POS_FRAMES, frame_number)
                            self.logger.debug(
                                f"_seek_player: Reset position after read (was {actual_frame_after}, set to {frame_number})")
            except Exception as e:
            ...
            self.logger.debug(
                f"_seek_player: Set OpenCV position to frame {frame_number} (before read: {actual_frame_before}, final: {actual_frame_final}), fps={fps}, position_ms={position_ms}")
            ...
            self.logger.debug(
                f"_seek_player: Timer state after seek - was_active={timer_state_before}, is_active={timer_state_after}, _is_playing={player._is_playing}, cap_opened={player.cap.isOpened() if player.cap else False}")
...
class TimelineWidget(QWidget):
    ...
    def set_time_range(self, start_time: datetime.datetime, end_time: datetime.datetime,
                       recording_segments: List[Tuple] = None):
...
class SplitVideoPlayerWidget(QWidget):
    ...
    def set_split_config(self, split_config: Dict, video_path: str):
        ...
        self.logger.info(
            f"Split config: num_split={num_split}, src_coords count={len(src_coords)}, source_names={source_names}")
        ...
        for i in range(num_split):
            ...
            region_container.setSizePolicy(QSizePolicy.Policy.Preferred, QSizePolicy.Policy.Preferred)
            region_container.setStyleSheet("border: 2px solid #888888;")
            region_container.setMinimumSize(100,
                                            100)  # Минимальный размер для предотвращения слишком маленьких виджетов
            region_layout = QVBoxLayout(region_container)
            region_layout.setContentsMargins(0, 0, 0, 0)
            region_layout.setSpacing(0)
    ...
    def _update_split_frames_opencv(self):
        ...
        if not cap or not cap.isOpened():
            self.logger.debug(
                f"_update_split_frames_opencv: cap is None or not opened. cap={cap}, isOpened={cap.isOpened() if cap else False}")
            return
        ...
        if hasattr(self._video_player, '_current_frame') and self._video_player._current_frame is not None:
            frame = self._video_player._current_frame.copy()
            self.logger.debug(
                f"_update_split_frames_opencv: Got frame from _current_frame, size={frame.shape if frame is not None else None}")
    ...
    def _split_frame(self, frame):
        ...
        if not self._split_config or not self._region_widgets:
            self.logger.warning(
                f"_split_frame: Missing config or widgets. config={self._split_config is not None}, widgets={len(self._region_widgets) if self._region_widgets else 0}")
            return
        ...
        try:
            ...
            for idx, region_info in enumerate(self._region_widgets):
                ...
                if w <= 0 or h <= 0:
                    self.logger.warning(
                        f"_split_frame: Region {idx} ({source_name}) has invalid dimensions after clipping: w={w}, h={h}")
                    continue
    ...
    def _display_region(self, widget: QLabel, region, source_name: str = "unknown"):
        ...
        if widget_size.width() > 0 and widget_size.height() > 0:
            ...
            self.logger.warning(
                f"_display_region ({source_name}): Widget size is invalid: {widget_size.width()}x{widget_size.height()}, cannot scale pixmap")
    ...
    def play(self):
        ...
        self.logger.info(
            f"play(): _video_player exists, _is_playing={getattr(self._video_player, '_is_playing', 'unknown')}")
        ...
        if hasattr(self._video_player, 'player') and self._video_player.player:
            ...
            if not self._video_player.timer.isActive():
                ...
                self.logger.info(
                    f"play(): VideoPlayerWidget timer started (before={timer_before}, after={timer_after}, interval={interval})")
            else:
                    ...
                    if self._video_player.cap.isOpened():
                        ...
                        if isinstance(ret, tuple) and len(ret) == 2 and ret[0]:
                            ...
                            if actual_frame_after != frame_number:
                                self._video_player.cap.set(cv2.CAP_PROP_POS_FRAMES, frame_number)
                                self.logger.debug(
                                    f"seek(): Reset position after read (was {actual_frame_after}, set to {frame_number})")
                except Exception as e:
