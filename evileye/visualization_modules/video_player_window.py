...
class VideoPlayerWidget(QWidget):
    ...
    def __init__(self, parent=None, logger_name: str | None = None, parent_logger: logging.Logger | None = None):
        ...
        try:
            disable_qt_mm = os.environ.get("EVILEYE_DISABLE_QT_MULTIMEDIA", "").strip().lower() in {"1", "true", "yes",
                                                                                                    "on"}
        except Exception:
            disable_qt_mm = False
    ...
    def _on_player_error(self, error, error_string=""):
        ...
        if should_fallback:
            self.logger.warning(
                f"QMediaPlayer/FFmpeg error detected (FFmpeg error: {error_msg}). Trying OpenCV fallback...")
            ...
            if status == QMediaPlayer.MediaStatus.EndOfMedia:
                ...
                if self.player and self.player.error() != QMediaPlayer.Error.NoError:
                    ...
                    if ("failed setup for format cuda" in error_lower or
                            ...
                            ("video width" in error_lower and "not within range" in error_lower)):
                        self.logger.warning(
                            f"CUDA/hardware acceleration error detected during loading: {error_str}. Switching to OpenCV fallback...")
                        if self.video_path:
                            self._use_opencv = True
                            self.play_video(self.video_path)
    ...
    def _update_frame_opencv(self):
        ...
        self.logger.debug(
            f"_update_frame_opencv: Entry - _is_playing={self._is_playing}, timer_active={timer_was_active}, cap_opened={cap_opened}, source={getattr(self, '_source_name', 'unknown')}")

        if not self._is_playing:
            if timer_was_active:
                self.logger.warning(
                    f"_update_frame_opencv: Timer was active but _is_playing=False, stopping timer (source={getattr(self, '_source_name', 'unknown')})")
            return
        ...
        if not self.cap or not self.cap.isOpened():
            ...
            if timer_was_active:
                self.logger.warning(
                    f"_update_frame_opencv: Timer was active but cap not opened (cap={cap_exists}, isOpened={cap_opened}), stopping timer (source={getattr(self, '_source_name', 'unknown')})")
            if self.timer:
                self.timer.stop()
            return
        ...
        if not ret:
            ...
            if not ret:
                self.logger.warning(
                    f"_update_frame_opencv: Failed to read frame even after restart, stopping timer (source={getattr(self, '_source_name', 'unknown')})")
                if self.timer:
                    self.timer.stop()
                return
    ...
    def play_video(self, video_path: str):
        ...
        if not self._use_opencv and self._supported_mime_types:
            ...
            if not self._is_mime_type_supported(mime_type):
                self.logger.info(
                    f"MIME type {mime_type} not supported by QMediaPlayer, using OpenCV fallback: {video_path}")
                self._use_opencv = True
            ...
            try:
                ...
                if cap_test.isOpened():
                    ...
                    if not ret or frame is None:
                        ...
                        self.logger.warning(
                            f"Video file appears corrupted or incomplete (cannot read frames), using OpenCV fallback: {video_path}")
                        cap_test.release()
                        self._use_opencv = True
                    else:
        ...
        if self._use_opencv:
            ...
            try:
                ...
                if not ret or frame is None:
                    self.logger.warning(
                        f"Video file appears to be corrupted or incomplete (cannot read frames): {video_path}")
                    self.cap.release()
                    self.cap = None
                    return False
                # Reset to beginning
                ...
                self.logger.debug(
                    f"play_video: Timer started (was_active={timer_was_active}, is_active={timer_is_active}, interval={interval}ms, fps={fps}, source={getattr(self, '_source_name', 'unknown')})")
            ...
            try:
                if pyqt_version == 6:
                    ...
                    if self.player.error() != QMediaPlayer.Error.NoError:
                        ...
                        if ("failed setup for format cuda" in error_lower or
                                ...
                                ("video width" in error_lower and "not within range" in error_lower)):
                            self.logger.warning(
                                f"CUDA/hardware acceleration error detected: {error_str}. Trying OpenCV fallback...")
                        else:
                            self.logger.warning(
                                f"QMediaPlayer error after setSource: {error_str}, path={video_path}. Trying OpenCV fallback...")
                    ...
                    def check_errors_after_play():
                        if self.player and self.player.error() != QMediaPlayer.Error.NoError:
                            ...
                            if ("failed setup for format cuda" in error_lower or
                                    ...
                                    ("video width" in error_lower and "not within range" in error_lower)):
                                self.logger.warning(
                                    f"CUDA/hardware acceleration error detected after play: {error_str}. Switching to OpenCV fallback...")
                                self.player.stop()
                                self._use_opencv = True
                                if self.video_path:
                                    self.play_video(self.video_path)
                    ...
                    if self.player.error() != QMediaPlayer.NoError:
                        error_str = self.player.errorString()
                        self.logger.warning(
                            f"QMediaPlayer error: {error_str}, path={video_path}. Trying OpenCV fallback...")
                        self.player.stop()
                        # Fallback to OpenCV if QMediaPlayer fails
                        self._use_opencv = True
                        return self.play_video(video_path)
                ...
                self.logger.warning(
                    f"Error playing video with QMediaPlayer: {e}, path={video_path}. Trying OpenCV fallback...")
                # Fallback to OpenCV
                self._use_opencv = True
                return self.play_video(video_path)
