...
class ObjectsJournalJson(QWidget):
    ...
    def _build_ui(self):
        ...
        self.image_delegate = events_journal_json.ImageDelegate(self.table, self.base_dir, logger_name="image_delegate",
        ...
        def _open():
            try:
                ...
                if not os.path.exists(img_path_abs):
                    ...
                    alt = os.path.join(self.base_dir, 'Detections', date_folder, 'Images', 'FoundFrames',
                                       os.path.basename(img_path))
                    if not os.path.exists(alt):
                        alt = os.path.join(self.base_dir, 'Detections', date_folder, 'Images', 'LostFrames',
                                           os.path.basename(img_path))
                    if not os.path.exists(alt):
                        # Legacy path
                    ...
                    if os.path.exists(alt):
                        ...
                        self.logger.warning(
                            f"ImageWindow: image not found: {img_path} (checked {img_path_abs} and {alt})")
                        return
                ...
                try:
                    ...
                    candidates.append(
                        img_path_abs.replace('/found_previews/', '/found_frames/').replace('_preview.', '_frame.'))
                    candidates.append(
                        img_path_abs.replace('/lost_previews/', '/lost_frames/').replace('_preview.', '_frame.'))
                    # Legacy detected_previews to found_frames
                    candidates.append(
                        img_path_abs.replace('detected_previews', 'found_frames').replace('_preview.', '_frame.'))
                ...
                try:
                    ...
                    if ev:
                        ...
                        cls = ev.get('class_name', ev.get('class_id', ''))
