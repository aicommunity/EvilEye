...
class JobsHistory(QWidget):
    ...
    def _setup_model(self):
        ...
        class ConfigHistoryTableModel(QAbstractTableModel):
            def __init__(self, parent=None):
                super().__init__(parent)
                self.data_list = []
                self.headers = ['Project ID', 'Job ID', 'Config ID', 'Status', 'Creation Time', 'Finish Time',
                                'Duration', 'Frames', 'Objects', 'Events', 'Configuration Info']
                ...
                if self.config_history_manager:
                    ...
                    if result.get('success'):
                        ...
                        QMessageBox.critical(self, "Ошибка",
                                             f"Не удалось создать проект: {result.get('error', 'Неизвестная ошибка')}")
                else:
            ...
            try:
                if self.config_history_manager:
                    ...
                    if result.get('success'):
                        ...
                        QMessageBox.critical(self, "Ошибка",
                                             f"Не удалось обновить проект: {result.get('error', 'Неизвестная ошибка')}")
                else:
                    QMessageBox.warning(self, "Ошибка", "ConfigHistoryManager не инициализирован")
            except Exception as e:
            ...
            try:
                if self.config_history_manager:
                    ...
                    if result.get('success'):
                        ...
                        QMessageBox.critical(self, "Ошибка",
                                             f"Не удалось удалить проект: {result.get('error', 'Неизвестная ошибка')}")
                else:
                    QMessageBox.warning(self, "Ошибка", "ConfigHistoryManager не инициализирован")
            except Exception as e:
    ...
    def _update_project_statistics(self):
        ...
        try:
            if not self.current_project_id or not hasattr(self,
                                                          'config_history_manager') or not self.config_history_manager:
