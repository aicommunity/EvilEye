        ...
        if self.file_types and path.is_file():
            if not any(str(path).lower().endswith(ext.lower()) for ext in self.file_types):
                return ValidationResult(False,
                                        f"{self.field_name}: Неподдерживаемый тип файла. Ожидается: {', '.join(self.file_types)}")
    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        try:
            ...
            if self.min_length is not None and len(parsed_list) < self.min_length:
                return ValidationResult(False,
                                        f"{self.field_name}: Список должен содержать минимум {self.min_length} элементов")

            if self.max_length is not None and len(parsed_list) > self.max_length:
                return ValidationResult(False,
                                        f"{self.field_name}: Список должен содержать максимум {self.max_length} элементов")
                ...
                for key, expected_type in self.schema.items():
                    if key in parsed_json:
                        if not isinstance(parsed_json[key], expected_type):
                            return ValidationResult(False,
                                                    f"{self.field_name}: Ключ '{key}' должен быть типа {expected_type.__name__}")
...
class Validators:
    ...
    # Списки
    SOURCE_IDS = ListValidator("ID источников", NumericValidator(integer_only=True, allow_negative=False),
                               allow_empty=False)
    FPS_LIST = ListValidator("Список FPS", FPS, allow_empty=False)

    # JSON
    BOTSORT_CONFIG = JSONValidator("Конфигурация BoTSORT",
                                   required_keys=['track_high_thresh', 'track_low_thresh', 'new_track_thresh',
                                                  'match_thresh'])
