...
function formatJournalCell(value: unknown): string {
...
function applyPreviewOverlay(
  ...
  imgSelector = '.journal-preview-img',
): void {
  const img = inner.querySelector<HTMLImageElement>(imgSelector);
  if (!img || !img.naturalWidth) return;
  let wrap = inner.querySelector<HTMLElement>('.journal-preview-overlay-wrap');
  if (!wrap) {
...
function frameUrl(row: JournalGroupedRow, journalType: JournalType, mode: 'found' | 'lost'): string {
  const path = previewImagePath(row, mode);
  if (!path) return '';
...
function tableImageUrl(row: JournalGroupedRow, journalType: JournalType, mode: 'found' | 'lost'): string {
  return previewUrl(row, journalType, mode) || frameUrl(row, journalType, mode);
}

function imageUrlFallbacks(
...
function mountDetailImage(
  ...
  const wrap = body.querySelector<HTMLElement>('.journal-detail-media-wrap');
  const img = body.querySelector<HTMLImageElement>('.journal-detail-media');
  if (!wrap || !img) return;

  let urlIndex = 0;
  const applyOverlay = () =>
    applyPreviewOverlay(wrap!, bbox as number[] | null, zone as number[][] | null, '.journal-detail-media');
  const tryNext = () => {
    urlIndex += 1;
  ...
  if (img.complete && img.naturalWidth > 0) applyOverlay();
}
...
function renderTableRow(
  ...
  const cells = columns.map((col) => {
    ...
    if (col.key === 'time' || col.key === 'time_lost') {
      return `<td>${formatJournalCell(item[col.key as keyof JournalGroupedRow])}</td>`;
    }
...
export function scrollJournalToTopIfFollowing(container: HTMLElement): void {
  const scroller = scrollContainerFor(container);
  if (scroller.scrollTop <= 50) scroller.scrollTop = 0;
}
...
export function prependJournalRows(
  container: HTMLElement,
  newRows: JournalGroupedRow[],
  journalType: JournalType,
  columns: Array<{ key: string; label: string; preview?: boolean }>,
): void {
  const tbody = container.querySelector('tbody');
  if (!tbody || !newRows.length) return;
...
function mountDetailVideo(
  ...
  const tryPlay = (path: string, fallback?: string) => {
    if (!path) {
    ...
    const video = body.querySelector<HTMLVideoElement>('.journal-detail-media');
    if (!video) return;
    video.addEventListener(
      'loadedmetadata',
      () => {
        if (offsetSec > 0) video.currentTime = offsetSec;
      },
      { once: true },
    );
    video.addEventListener(
...
export interface JournalDetailOptions {
  initialTab?: 'video' | 'found' | 'lost';
}

export function openJournalDetailModal(
  row: JournalGroupedRow,
  journalType: JournalType,
...
async function ensureRowBbox(row: JournalGroupedRow, journalType: JournalType): Promise<JournalGroupedRow> {
  if (row.bbox_found != null || row.bbox_lost != null) return row;
  try {
...
function updatePreviewCell(inner: HTMLElement, row: JournalGroupedRow, journalType: JournalType, mode: 'found' | 'lost'): void {
  ...
  if (img && url) {
    img.onload = () => {
      const bbox = mode === 'found' ? row.bbox_found : row.bbox_lost;
      const zone = mode === 'found' ? row.zone_coords : null;
      applyPreviewOverlay(inner, bbox as number[] | null, zone as number[][] | null);
    };
    img.src = url;
  }
...
export function mountJournalPreviewCells(container: HTMLElement, rowsByKey?: Map<string, JournalGroupedRow>): void {
  container.querySelectorAll<HTMLElement>('.journal-preview-inner').forEach((inner) => {
    ...
    const applyOverlayForRow = (r: JournalGroupedRow) => {
      const mode = (inner.dataset.mode as 'found' | 'lost') || 'found';
    ...
    inner.querySelector('.journal-preview-img')?.addEventListener('dblclick', () => {
      if (row) openJournalDetailModal(row, journalType);
    });
